...
from .models import Experience, Education, Skill, Project, About, Article, CV, Certificate, MainCertificate, Publication

# Exclude the AnalyticsMiddleware because background threading in SQLite causes database lock issues during tests
...
class ViewFunctionalTests(TestCase):
    ...
    def test_index_view_post_contact_form_ajax(self, mock_getenv):
        ...
        response = self.client.post(reverse('index'), data, HTTP_X_REQUESTED_WITH='XMLHttpRequest')
        self.assertEqual(response.status_code, 200)
