...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if certificate.objects.exists():
            self.stdout.write(
                self.style.WARNING(
                    "Certificates already exist in the database. Skipping import."
                )
            )
            return
        ...
        try:
            with open(csv_path, newline="", encoding="utf-8") as csvfile:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully created {certificates_created} certificates"
                    )
                )
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Error loading certificates: {str(e)}"))
