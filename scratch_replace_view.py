...
new_code = '''from django.utils.decorators import method_decorator
...
with io.open(r'd:\viir_portfolio\viir_folio\views.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Replace from `@ratelimit` down to the next function `def certificate_view`
pattern = re.compile(r'@ratelimit\(key=\'ip\', rate=\'3/5m\', method=\'POST\', block=False\)\s*def index\(request\):.*?(?=\n\s*def certificate_view\(request\):)', re.DOTALL)

if pattern.search(content):
    content = pattern.sub(new_code + '\n\n', content)
    with io.open(r'd:\viir_portfolio\viir_folio\views.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print('Replaced index view successfully.')
else:
    print('Could not find index view pattern.')
